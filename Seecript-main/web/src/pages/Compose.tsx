...
import { StoryBoard } from '@/components/compose/StoryBoard'
...
const ACTION_TABS: { value: FillAction; label: string; hint: string }[] = [
  { value: 'rerank', label: '智能推荐', hint: '从你的素材里自动挑出最合适的一个' },
  { value: 'copy', label: '文字卡片', hint: 'AI 为你设计一张个性化文字卡片用作画面' },
  { value: 'aigc', label: 'AI 视频', hint: 'AI 生成一段 5-8 秒短片用作画面' },
  { value: 'aigc_image', label: 'AI 图片转动画', hint: 'AI 生成图片后转为动画镜头，成本更低等待更短' },
]

const RENDER_STEP_LABELS: Record<string, string> = {
  prepare: '准备中',
  ffmpeg_concat: '画面合成',
  seedance_extend: 'AI 画面生成',
  remotion_render: '添加字幕与特效',
  ffmpeg_overlay: '导出成片',
  finalize: '收尾',
}
...
export default function ComposePage() {
   ...
   *  就置 true，「AI 生成内容方案」按钮才解禁。state 仅活在当前会话内，刷页面会重置——
  ...
  // 用户流：plan 出来 → 手动 / NL 编辑 → 补齐所有缺口 → 点「确认方案，预览效果」 →
  ...
  // 步骤 3 解锁 gate：plan 生成完了只解锁 step 2；用户在 step 2 点「继续下一步」才置 true。
  // plan_id 变更复位规则：
  ...
  /* --------------------- 渲染流水线（内联 · 无独立页面）--------------------- */
  // 设计：用户点「导出视频」之后，先补缺口 + 生成包装 + commit compose，再自动 POST /render/submit
  ...
  if (!selectedSampleId) {
    return (
      <PageShell title="创作工作台" subtitle="选参考、写主题、配素材、出片——一站式内容创作。">
  ...
  return (
    <PageShell
      title="创作工作台"
      subtitle="第 1 步选参考 + 写主题 → 第 2 步 AI 生成内容方案 → 第 3 步出片。"
    >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="rounded border border-border bg-background px-2 py-0.5 text-xs hover:bg-secondary disabled:opacity-50"
                    title="从系统素材库选择补充"
                  >
                    + 从系统素材库
                  </button>
                  <span className="text-xs text-muted-foreground">{sortedMaterials.length} 条</span>
                </div>
              </div>
              <MaterialGrid
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          ...
          {plan && !analyzing && (
            <button
              ...
              className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-secondary hover:-translate-y-px active:scale-[0.98] transition-all duration-200"
            >
              直接进入第 2 步（不重生成）
            </button>
          )}
        ...
        >
          {analyzing ? (
            <div className="flex max-w-md flex-col items-center gap-3 rounded-lg border border-border bg-card px-8 py-6 text-center shadow-2xl">
              ...
              <div className="text-base font-semibold text-foreground">正在 AI 生成内容方案…</div>
                ...
                <div className="flex items-center gap-2">
                  {(plan.kb_rules_applied ?? 0) > 0 && (
                    <button
                      ...
                      title="点击去个性知识库管理"
                      className="flex items-center gap-1 rounded-full border border-primary/30 bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary hover:bg-primary/20"
                    >
                      已应用 {plan.kb_rules_applied} 条规则 · 去管理 →
                    </button>
                  )}
                  <span className="text-xs text-muted-foreground">{videoType}</span>
                </div>
              </div>
      ...
      {activeStep === 2 && plan && (
        <section className="mt-4 space-y-3 rounded-lg border border-border bg-card p-4">
          ...
          <div className="space-y-2 border-t border-border pt-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                className={cn(
                  'rounded-md border border-border bg-card px-2 py-1 text-xs hover:bg-secondary',
          ...
          <div className="flex items-center justify-between gap-2 border-t border-border pt-3">
            ...
            <div className="flex items-center gap-2">
              {fills.length > 0 && (
                <span className="text-xs text-muted-foreground">已采纳 {fills.length}</span>
              )}
              <BatchCopyButton
                planId={plan.plan_id}
          ...
          <div className="grid gap-3 lg:grid-cols-[1fr_1.2fr]">
            ...
            {/* 右 · 缺口补全（依赖选中 gap） */}
            <div className="space-y-2">
              <p className="rounded-md border border-primary/30 bg-primary/5 px-2 py-1.5 text-xs leading-relaxed text-foreground">
          ...
          <div className="mt-3 flex flex-col gap-2 border-t border-border pt-3 sm:flex-row">
            ...
            >
              {batchFillBusy
                ? '一键补字卡进行中…内容轨同步后才能继续下一步'
                : analyzing
                  ? '内容轨重排中…'
                  : mainTrackUnfilledCount > 0
                    ? `内容轨还有 ${mainTrackUnfilledCount} 段未补完 · 补齐后继续下一步`
                    : pendingGapsCount > 0
                      ? `还有 ${pendingGapsCount} 段缺口待补 · 补齐后继续下一步`
                      : trackBusy
                        ? '准备中…（重写口播 / 配音 / 包装推荐）'
                        : '继续下一步 → 解锁口播 / 包装 / BGM 与实时预览'}
            </button>
      ...
      <section className="mt-4">
        {plan ? (
          <>
            <div className="mb-2 flex items-center gap-2 rounded-md border border-border bg-background/40 px-3 py-1.5 text-xs text-muted-foreground">
              <span>第 3 步 · 实时预览与三轨已展开。包装方案入口已移到下方"包装轨"右上角。</span>
              <button
                type="button"
                onClick={() => setActiveStep(2)}
                className="ml-auto rounded-md border border-border bg-background px-2 py-0.5 text-xs hover:bg-secondary"
              >
                ← 返回第 2 步
              </button>
            </div>
          ...
          <div className="rounded-md border border-dashed border-border bg-background/30 p-6 text-center text-xs text-muted-foreground">
            还没生成方案。回到第 1 步填写主题，点「AI 生成内容方案」开始。
          </div>
        )}
      </section>
      )}
      ...
      {/* ============ Row 6：一键导出视频（步骤 3 专属）============ */}
      {activeStep === 3 && plan && (
        <section className="mt-6 space-y-3 rounded-xl bg-secondary/40 p-4">
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="text-sm font-semibold">导出视频</h2>
            <button
              ...
              title="先用文案补全所有未补缺口，再生成封面与转场（转场 + 封面），最后直接渲染成片"
              className={cn(
            ...
            >
              {finalizing === 'filling-gaps' && '正在补齐剩余素材…'}
              {finalizing === 'packaging' && '生成封面与转场中…'}
              {isRendering && `渲染中 · ${renderPercent}%`}
              {!isRendering &&
                finalizing !== 'filling-gaps' &&
                finalizing !== 'packaging' &&
                (renderDone ? '重新导出视频' : '一键导出视频')}
            </button>
            <button
              onClick={() => setActiveStep(2)}
              disabled={finalizing === 'filling-gaps' || finalizing === 'packaging'}
              className="rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium hover:bg-secondary hover:-translate-y-px active:scale-[0.98] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              ← 返回上一步调整内容
            </button>
        ...
        <section className="mt-4 flex flex-col items-end gap-2">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={!canUndo}
              className={cn(
                'rounded-lg border border-border bg-background px-3 py-1 text-xs font-medium hover:bg-secondary hover:-translate-y-px active:scale-[0.98] transition-all duration-200',
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-4 py-3">
              <h3 className="text-sm font-semibold">结构对照：参考样本 → 你的方案</h3>
              <button
                onClick={() => setStructureZoomOpen(false)}
...
function StructureCompareSection({
  ...
  return (
    <section className="rounded-xl bg-secondary/40">
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <h3 className="text-sm font-semibold">
            结构对照：{secondaryManifest ? '样本1 → 你的方案 ← 样本2' : '样例 → 新方案'}
          </h3>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded-lg border border-border bg-background px-2 py-1 text-xs font-medium hover:bg-secondary hover:-translate-y-px active:scale-[0.98] transition-all duration-200"
...
function WorkshopStepNav({
  ...
  const step3Reason = !hasPlan
    ...
    : batchFillBusy
      ? '一键补字卡进行中，完成后才能继续下一步'
      : mainTrackUnfilledCount > 0
        ? `内容轨还有 ${mainTrackUnfilledCount} 段未补完，请先在第 2 步补齐再继续下一步`
        : !step3Unlocked
          ? '请在第 2 步底部点「继续下一步」解锁'
          : pendingGapsCount > 0
            ? `还有 ${pendingGapsCount} 个缺口未补，可继续继续下一步自动补全`
            : ''
      ...
      {steps.map((s) => {
        ...
        return (
          <button
            ...
            className={cn(
              'rounded-xl px-3 py-2 text-left transition-all duration-300',
              active
                ? 'selected-linear shadow-sm'
                : clickable
                  ? 'bg-card hover:bg-primary/5 hover:ring-1 hover:ring-primary/20'
          ...
          >
            ...
            {s.disabled && s.tip && (
              <div className="mt-1 text-xs text-amber-600 dark:text-amber-400">{s.tip}</div>
            )}
          </button>
        )
      })}
  ...
  return (
    <div>
      ...
      <div className="h-1.5 overflow-hidden rounded-full bg-secondary">
        <div
          className="h-full bg-primary transition-all duration-300 animate-shimmer"
          style={{ width: `${Math.min(100, percent)}%` }}
        />
      </div>
      <ol className="mt-2 grid grid-cols-3 gap-1 text-xs sm:grid-cols-6">
        {RENDER_STEP_ORDER.map((s, i) => (
          <li
            key={s}
            className={cn(
              'rounded border px-1 py-0.5 text-center transition-all duration-300',
              i < currentIdx
                ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
                : i === currentIdx
                  ? 'border-primary bg-primary/10 text-primary animate-step-pulse'
...
function UploadDropzone({
  ...
  return (
    <div
      ...
      onClick={onPick}
      className={cn(
        'flex h-24 cursor-pointer items-center justify-center rounded-xl border-2 border-dashed text-sm transition-colors',
        hover ? 'selected-linear' : 'border-border bg-background/40',
        uploading && 'pointer-events-none opacity-60',
      )}
    >
      <span className="text-muted-foreground">
        {uploading ? '上传中…' : '点击或拖拽上传视频 / 图片 / 音频（单文件 ≤ 50MB）'}
      </span>
    </div>
  )
}
