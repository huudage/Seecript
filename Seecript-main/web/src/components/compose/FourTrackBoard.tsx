...
function TextCardThumb({ spec }: { spec: TextCardSpec }) {
  ...
  return (
    ...
    >
      <span className="text-xs" style={{ letterSpacing: spec.font_family === 'tech_mono' ? '0.05em' : 'normal' }}>
        {head}
      </span>
...
function SceneThumb({
  ...
  // 兜底：source 类型字符
  const glyph = scene.shot_subject || (scene.scene_label || '场景').slice(0, 4)
  return (
    <div className="flex h-full w-full items-center justify-center bg-black/30 text-xs font-bold text-white/80">
      {glyph}
    </div>
  )
}
  ...
  return (
    <div className="relative space-y-2 overflow-hidden rounded-lg border border-border bg-card p-4">
      ...
      <div className="grid grid-cols-[88px_1fr] items-center">
        <span className="text-xs font-semibold text-muted-foreground">时间轴</span>
        <div
          className={cn(
        ...
        >
          {ticks.map((t) => (
            <span
              key={t}
              className="absolute top-0 -translate-x-1/2 text-xs font-mono text-muted-foreground"
            ...
            {mf.sections.map((sec, sIdx) => {
              ...
              return (
                <div
                  key={`ref-${idx}-${sIdx}`}
                  className={cn(
                    'absolute top-1 bottom-1 overflow-hidden rounded-md border border-border/50 text-xs text-white shadow-sm',
                    meta.bg,
                    'opacity-80',
                  )}
                ...
                >
                  <div className="flex h-full flex-col justify-between p-1">
                    <span className="font-mono text-xs opacity-80">{meta.short}</span>
        ...
        {contentTrackMode === 'sections' ? sectionBlocks.map((block) => {
          ...
          return (
            <button
              ...
              className={cn(
                'absolute top-1 bottom-1 overflow-hidden rounded-md border-2 text-left text-xs text-white shadow-sm transition-all',
            ...
            >
              ...
              <div className="relative z-[1] flex h-full flex-col justify-between p-1">
                <div className="flex items-center justify-between gap-1">
                  <span className="rounded bg-black/40 px-1 font-mono text-xs text-white">
                    {getSectionMeta(section.role).short}
                  </span>
                  <div className="flex items-center gap-1">
                    {scenesInSec.length > 1 && (
                      <span
                        className="inline-flex h-3 items-center justify-center rounded-full bg-violet-300/95 px-1 font-mono text-xs font-bold text-violet-900"
                    ...
                    {gap && (
                      <span
                        className={cn(
                          'inline-flex h-3 min-w-3 items-center justify-center rounded-full px-1 text-xs font-bold',
                          effectiveStatus === 'ok'
                ...
                <div className="truncate rounded bg-black/40 px-1 text-xs font-semibold leading-tight text-white">
      ...
      */}
      ...
      >
        {!subtitleEnabled ? (
          <div className="absolute inset-1 flex items-center justify-center rounded-md border border-dashed border-border bg-background/30 text-xs text-muted-foreground">
          ...
          scenes.map((scene) => {
            ...
            if (isTextCard) {
              ...
              return (
                <div
                  key={scene.scene_id}
                  className="absolute top-1 bottom-1 flex items-center justify-center overflow-hidden rounded-md border border-dashed border-border/60 bg-muted/40 text-xs text-muted-foreground"
                  style={{ left: `${left}%`, width: `calc(${width}% - 2px)` }}
            ...
            return (
              <div
                ...
                className={cn(
                  'absolute top-1 bottom-1 overflow-hidden rounded-md border text-left text-xs shadow-sm transition',
                  subText
              ...
              >
                <div className="flex h-full flex-col gap-0.5 px-1 py-0.5">
                  <span className="font-mono text-xs opacity-70">{scene.scene_id}</span>
            ...
            <div className="flex w-full flex-col gap-1">
              {onChangeTtsVoice && (
                <select
                  ...
                  className="w-full rounded border border-border bg-background/60 px-1 py-0.5 text-xs text-foreground outline-none focus:border-primary disabled:opacity-50"
                >
                  {VOICE_OPTIONS.map((v) => (
              ...
              {onSynthesizeAll && (
                <button
                  ...
                  disabled={busy}
                  className="rounded border border-primary/40 bg-primary/10 px-2 py-0.5 text-xs text-primary hover:bg-primary/20 disabled:opacity-50"
                    ...
                    <span className="ml-auto inline-flex shrink-0 gap-0.5">
                      {!readOnly && state !== 'empty' && onSynthesizeScene && (
                        <button
                          ...
                          disabled={busy}
                          className="rounded bg-background/60 px-1 text-xs hover:bg-background disabled:opacity-50"
              ...
              {onAddPackagingItem && (
                <div className="relative" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    className="rounded border border-border bg-background/70 px-1.5 py-0.5 text-xs text-muted-foreground hover:bg-secondary hover:text-foreground disabled:opacity-50"
                  >
                    {busy ? '⏳…' : '+组件 ▾'}
                  </button>
                  {addMenuOpen && (
      ...
      >
        {nonSubtitleItems.length === 0 && scenes.length < 2 ? (
          <div className="absolute inset-1 flex items-center justify-center rounded-md border border-dashed border-border bg-background/30 text-center text-xs text-muted-foreground">
            还没生成包装项（标题 / 转场 / 封面 / 贴纸）
          </div>
        ) : (
          <>
              ...
              scenes.slice(1).map((sc, idx) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'absolute top-1 z-10 inline-flex h-6 -translate-x-1/2 select-none items-center gap-0.5 rounded-full border border-white/60 px-1.5 text-xs font-semibold shadow-sm',
                      tone,
            ...
            {nonSubtitleItems.map((it, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'group absolute top-1 bottom-1 flex items-center justify-center overflow-hidden rounded text-xs font-medium shadow transition',
                ...
                >
                  ...
                  {canDelete && (
                    <span
                      ...
                      title="删除该包装项"
                      className={cn(
                        'absolute right-0.5 top-0.5 inline-flex h-3.5 w-3.5 cursor-pointer items-center justify-center rounded-full bg-black/40 text-xs font-bold text-white opacity-0 transition-opacity hover:bg-rose-500 group-hover:opacity-100',
              ...
              <div className="flex items-center gap-1">
                ...
                {bgm?.track_url && onClearBgm && (
                  <button
                    ...
                    disabled={busy}
                    className="rounded border border-border bg-background/60 px-2 py-0.5 text-xs text-muted-foreground hover:bg-background disabled:opacity-50"
                  >
                    清除
                  </button>
                )}
...
function TrackRow({
  ...
  return (
    <div className="grid grid-cols-[88px_1fr] items-stretch gap-1">
      <div className="flex flex-col items-start justify-center gap-0.5 pr-1 text-xs">
  ...
  return (
    <div className="absolute inset-1 select-none">
      ...
      >
        ...
        {hasAnalysis && climaxes.map((hl, idx) => {
          ...
          return (
            <span
              key={`hl-${idx}`}
              className={cn(
                'absolute -top-2 -translate-x-1/2 rounded px-1 text-xs font-bold shadow',
        ...
        {peakRel != null && (
          <span
            className="absolute -top-2 -translate-x-1/2 rounded bg-rose-500 px-1 text-xs font-bold text-white shadow"
            style={{ left: `${peakRel}%` }}
        ...
        <span className="pointer-events-none absolute inset-0 flex items-center justify-center text-xs font-semibold text-violet-900 dark:text-violet-100">
