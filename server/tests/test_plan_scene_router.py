...
# ---------------------------------------------------------------------------
# stage-66 不变量：scene.duration 永远 = (out_point - in_point)
# 用户原话（2026-06-11）："裁哪段如实播放哪一段即可"——
# 渲染端不再做 freeze/slow-mo 凑长度，所以 mismatch 会变成尾巴黑屏 → 必须从源头杜绝。
# ---------------------------------------------------------------------------

def _put_test_material_with_shots(project_id: str, material_id: str, duration: float,
                                   shot_specs: list[tuple[float, float]]) -> None:
    """造一条带 PySceneDetect shots 的视频素材。shot_specs 是 [(start, end), ...]。"""
    from app.schemas import Material, MaterialShot
    from app.services.materials.store import material_store

    shots = [
        MaterialShot(index=i, start=s, end=e, duration=round(e - s, 3))
        for i, (s, e) in enumerate(shot_specs)
    ]
    mat = Material(
        material_id=material_id,
        filename=f"{material_id}.mp4",
...
def test_swap_source_shot_index_shrinks_scene_duration_when_shot_shorter(client):
    """素材 shot 比 scene.duration 短：scene.duration 必须缩到 (out-in)，
    不能保留原 duration 让渲染端 freeze 尾帧——那就是用户说的『片段重复』。"""
    project_id = f"proj-shot-short-{int(time.time() * 1000)}"
    plan_id = f"plan-shot-short-{int(time.time() * 1000)}"
    # 素材有 2 个 shot：第 0 个仅 1.5s（比 sc-0 原 duration 3.0 短）
    _put_test_material_with_shots(project_id, "mat-shots", duration=10.0,
                                  shot_specs=[(0.0, 1.5), (1.5, 5.0)])
    plan = _make_plan_with_project(plan_id, project_id)
    _TEST_PLAN_IDS.append(plan_id)
    plan_store.put(plan)
    ...
    try:
        resp = client.post(
            ...
            json={
                "source": "user_material",
                "material_id": "mat-shots",
                "material_shot_index": 0,
            },
        )
        assert resp.status_code == 200, resp.text
        body = resp.json()
        sc0 = next(s for s in body["main_track"] if s["scene_id"] == "sc-0")
        assert sc0["in_point"] == 0.0
        assert abs(sc0["out_point"] - 1.5) < 0.001
        # 关键不变量：duration == out - in
        assert abs(sc0["duration"] - 1.5) < 0.001, \
            f"shot 比 scene 短时 duration 必须跟随窗口缩短；期望 1.5，实际 {sc0['duration']}"
        # plan.duration_seconds 同步收缩（sc-0 由 3 → 1.5，sc-1 由 4 → 4）
        assert abs(body["duration_seconds"] - 5.5) < 0.001
    finally:
        _drop_test_material(project_id)


def test_swap_source_no_shots_clamps_duration_to_material_length(client):
    """素材没切镜且整片比 scene.duration 短：duration 缩到素材实际时长。"""
    project_id = f"proj-mat-short-{int(time.time() * 1000)}"
    plan_id = f"plan-mat-short-{int(time.time() * 1000)}"
    _put_test_material(project_id, "mat-tiny", duration=1.2)  # < sc-0 的 3.0
    plan = _make_plan_with_project(plan_id, project_id)
    _TEST_PLAN_IDS.append(plan_id)
    plan_store.put(plan)
    ...
    try:
        ...
        sc0 = next(s for s in body["main_track"] if s["scene_id"] == "sc-0")
        assert sc0["in_point"] == 0.0
        assert abs(sc0["out_point"] - 1.2) < 0.001
        assert abs(sc0["duration"] - 1.2) < 0.001, \
            f"素材短于目标时 scene.duration 必须收缩，期望 1.2，实际 {sc0['duration']}"
    finally:
        _drop_test_material(project_id)
...
def test_edit_scene_duration_syncs_user_material_out_point():
    ...
    from app.routers.edit import _dispatch_main

    plan = _make_plan(f"plan-edit-dur-{int(time.time() * 1000)}")
    plan.main_track[0].in_point = 1.0
    plan.main_track[0].out_point = 4.0
    plan.main_track[0].duration = 3.0  # 一致：3.0 == 4.0 - 1.0

    _dispatch_main(plan, "edit_scene_duration", {"scene_id": "sc-0", "duration": 6.0})

    sc0 = plan.main_track[0]
    assert abs(sc0.duration - 6.0) < 0.001
    assert sc0.out_point is not None
    assert abs((sc0.out_point - sc0.in_point) - sc0.duration) < 0.001, \
        f"out-in 必须跟 duration 同步；in={sc0.in_point} out={sc0.out_point} dur={sc0.duration}"
    assert abs(sc0.out_point - 7.0) < 0.001  # 1 + 6


def test_edit_scene_duration_skips_aigc_out_point():
    """非 user_material 不应被这条新逻辑误填 out_point。"""
    from app.routers.edit import _dispatch_main

    plan = _make_plan(f"plan-edit-dur-aigc-{int(time.time() * 1000)}")
    plan.main_track[0].source = "aigc_image"
    plan.main_track[0].in_point = 0.0
    plan.main_track[0].out_point = None

    _dispatch_main(plan, "edit_scene_duration", {"scene_id": "sc-0", "duration": 5.0})

    assert plan.main_track[0].out_point is None
    assert abs(plan.main_track[0].duration - 5.0) < 0.001
