...
| 3 · 素材上传 + 缺口识别 | **砍** | 多模态自动打标 + 槽位匹配落位整体移除——超出 AI 能力边界（识别 / 匹配精度撑不起默认生效），同时越界用户自主性（素材放哪段是创作决策，AI 不替用户拍板）。v2 落位回归拖拽直接操纵；缺口以画布空槽直接可见；不确定时中键「AI 理解」按需出贴纸（不落 plan，可摘除） |
...
| D5 | 缺口 / 补拍 | 空槽即缺口 + 四路补全（rerank / 字卡 / AIGC 图 / 补拍清单） | 空槽中键打开字卡 / AIGC 图 / 补拍清单；实拍换槽仍是拖拽。T2V 只在盘内「AI 合成视频」 |
| D6 | 渲染线 | ffmpeg 原生成片 + WebAV 整体预览导出，移除 Remotion | 待开始 |
| D7 | 裁剪收窄 | 外围整体移除（含素材自动识别 / 情绪曲线）+ 上传收窄 + 回归 | 上传已收窄 ≤60s；知识库页已从导航移除；情绪曲线已从拆解、方案和页面拿掉。TTS / 步骤状态机后端仍在，渲染线未切完 |
