...
- **成片总览条**在画布底部按链序显示时长占比。时间轴和字幕 / 口播 / 音乐轨独立放在画布下面，不嵌进画布；
...
AI 入口收敛为**功能盘**：右键任何对象，盘跟随鼠标弹出该对象锚定的动作。盘 = 后端 tool 白名单的可视化——盘上有什么就是允许做什么，无隐藏能力、不摆灰置项。撤销、功能介绍、结构迁移对照放在画布工具条里，默认折叠；结构迁移只作参考，不给视频块标结构名，也不要求补满每一段。补全表单从盘里点开，在画布底边展开抽屉。⌘K 对话保留为键盘党的兜底。AI 的边界看得见，就越不了界；同时每个 AI 动作都有画布直接操纵等价物——AI 能配口播，人也能双击块内层直接改字；AI 能裁剪，人也能拖入出点。
...
| | v1 | v2 |
|---|---|---|
| U1 结构画布 | 四轨时间线工作台 | 画布：段落块 × 素材槽 × 连线；总览条在画布内。时间轴与轨道独立放在画布下 |
...
| D8 | 画布收口 | 右键唤盘；工具条放撤销 / 帮助 / 结构迁移对照（折叠，对照只作参考，块不标结构名，空槽不挡出片）；时间轴与轨道独立放在画布下 | ✅ 已落地 |
