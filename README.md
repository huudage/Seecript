...
AI 入口收敛为**功能盘**：右键任何对象，盘跟随鼠标弹出该对象锚定的动作。盘 = 后端 tool 白名单的可视化——盘上有什么就是允许做什么，无隐藏能力、不摆灰置项。撤销、功能介绍、结构迁移对照放在画布工具条里，默认折叠；结构迁移只作参考，不给视频块标结构名，也不要求补满每一段。内容轨不标开场钩子这类角色，视频块时长不设上下限。轨道放在画布顶部，可以收起。生成（字卡 / 图片 / 视频）在功能盘三个方向上，悬停展开。自然语言改片在盘里，右下角对话和导出后的自进化蒸馏都还在。画布这一步铺满视口，页面上不再提示适配结构或缺口。⌘K 对话保留为键盘党的兜底。AI 的边界看得见，就越不了界；同时每个 AI 动作都有画布直接操纵等价物——AI 能配口播，人也能双击块内层直接改字；AI 能裁剪，人也能拖入出点。
